...
describe("PUT /rule/:id", () => {
  it("should update a pricing rule", async () => {
    ...
    // 3. Update the pricing rule
    const updateRes = await request(app)
      .put(`/rule/${createdRuleId}`)
      .set("Authorization", `Bearer ${token}`)
      .send({
        type: "percentage",
        percentage: 25,
    ...
    expect(updateRes.body.data).toHaveProperty("id", createdRuleId);
    expect(updateRes.body.data).toHaveProperty("percentage", "25.00");
  });
});
